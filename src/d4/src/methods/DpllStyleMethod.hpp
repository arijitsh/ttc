...
class DpllStyleMethod : public MethodManager, public Counter<T> {
 ...
 public:
  ...
  DpllStyleMethod(const OptionDpllStyleMethod &options,
    ...
    m_allowDecompose = options.decompose;
    m_connectedComponent = m_allowDecompose;
 ...
 private:
  ...
  /**
   * @brief Learn a monotone clause from the current assumption stack.
   *
   * If the problem is unsatisfiable under the current assumptions and the
   * monotone optimisation is enabled, we can add a clause consisting of the
   * negation of all projection variables assigned to false.  This mimics the
   * behaviour implemented for the DDNNF counter.
   */
  void addMonotoneClause() {
    // Only learn a monotone clause when the underlying CNF was proven
    // unsatisfiable.  If the SMT layer refutes the assignment, the monotone
    // property does not necessarily hold and adding a clause could prune valid
    // solutions.
    if (!d4_use_mono || m_solver->lastCnfSat()) return;
    std::vector<Lit> clause;
    auto &assums = m_solver->getAssumption();
    for (const auto &l : assums) {
      if (l.sign() && m_isDecisionVariable[l.var()]) clause.push_back(~l);
    }
    if (!clause.empty()) {
  ...
  */
  ...
  inline int computeConnectedComponent(
    ...
    if (m_connectedComponent || !(m_nbCallCall % 500)) {
      double compStart = Log.elapsed();
      unsigned ret = m_specs->computeConnectedComponent(varConnected, setOfVar,
                                                        freeVariable);
      double compEnd = Log.elapsed();
      Profile.addComponent(compEnd - compStart);
    ...
    double compStart = Log.elapsed();
    int ret = m_specs->computeTrivialConnectedComponent(varConnected, setOfVar,
                                                        freeVariable);
    double compEnd = Log.elapsed();
    Profile.addComponent(compEnd - compStart);
    return ret;
   ...
   */
    U compute_(std::vector<Var> &setOfVar, std::vector<Lit> &unitsLit,
      ...
      cvc5::Term cacheKey;
      bool useExtCache = d4_component_cache != nullptr &&
                         !m_solver->getAssumption().empty() &&
                         !setOfVar.empty();
      bool needSmtContext = d4_use_component_cache && d4_smt_solver != nullptr &&
                            !setOfVar.empty();
      if ((useExtCache || needSmtContext) && d4_smt_solver) {
        auto& tm = ttc::getTermBuilder(*d4_smt_solver);
        cacheKey = tm.mkTrue();
        for (auto &a : m_solver->getAssumption()) {
          if (a.var() >= (int)d4_smt_idxToTerm.size()) continue;
          cvc5::Term t = d4_smt_idxToTerm[a.var()];
          if (a.sign()) t = tm.mkTerm(cvc5::Kind::NOT, {t});
          cacheKey = tm.mkTerm(cvc5::Kind::AND, {cacheKey, t});
        }
      }
      if (useExtCache) {
        Profile.addCacheLookup();
        Profile.addDecision();
        std::uint64_t cachedVal;
        if (d4_component_cache->lookup(cacheKey, cachedVal)) {
          Profile.addCacheHit();
          U ret = cachedVal;
          return ret;
        }
      }
      std::size_t smtContextHash = 0;
      if (needSmtContext && !cacheKey.isNull()) {
        smtContextHash = std::hash<cvc5::Term>{}(cacheKey);
      }

      if (!m_solver->solve(setOfVar)) {
        addMonotoneClause();
        if (useExtCache) {
          d4_component_cache->insert(cacheKey, 0);
        }
        if (Trace.isEnabled("d4")) {
          if (m_solver->lastCnfSat() && !m_solver->lastSmtSat())
            Trace("d4") << "stopped by SMT" << std::endl;
          else
            Trace("d4") << "CNF UNSAT" << std::endl;
        }
    ...
    // Keep only decision variables (projection vars) among free vars.
    expelNoDecisionVar(freeVariable, m_isDecisionVariable);
    // Important: Free decision variables must still be decided (not counted
    // as unconstrained 2-ways), otherwise projected counting overcounts.
    if (!freeVariable.empty()) {
      varConnected.push_back(freeVariable);
      freeVariable.clear();
      ++nbComponent;
    }

    if (Trace.isEnabled("decompose")) {
      if (nbComponent > 1)
        Trace("decompose") << "decompose " << nbComponent << std::endl;
      else
        Trace("decompose") << "no decomposition" << std::endl;
    }
    ...
    // consider each connected component.
      ...
      if (nbComponent) {
        ...
        double decompStart = 0.0;
        if (nbComponent > 1) decompStart = Log.elapsed();
        for (int cp = 0; cp < nbComponent; cp++) {
          ...
          if (cacheActivated && cb.defined) {
            if (needSmtContext) {
              auto bucketIt = d4_component_smt_values.find(cb.e.data);
              if (bucketIt != d4_component_smt_values.end()) {
                auto valueIt = bucketIt->second.find(smtContextHash);
                if (valueIt != bucketIt->second.end()) {
                  tab[cp] = static_cast<U>(valueIt->second);
                  handled = true;
                }
              }
              if (!handled) {
                tab[cp] = computeDecisionNode(connected, out);
                boost::multiprecision::mpz_int stored = tab[cp];
                d4_component_smt_values[cb.e.data][smtContextHash] = stored;
                handled = true;
              }
            } else {
              tab[cp] = cb.getValue();
        ...
        if (nbComponent > 1) {
          double decompEnd = Log.elapsed();
          Profile.addDecompose(decompEnd - decompStart);
        }
        result = m_operation->manageDecomposableAnd(tab, nbComponent);
      } else {
        // m_specs->postUpdate(additionalUnit);
        m_specs->postUpdate(unitsLit);
        expelNoDecisionLit(unitsLit, m_isDecisionVariable);
        result = m_operation->createTop();
      }
      if (useExtCache) {
        d4_component_cache->insert(cacheKey,
                                  result.template convert_to<std::uint64_t>());
      }
      return result;
    }  // compute_
  ...
  U computeDecisionNode(std::vector<Var> &connected, std::ostream &out) {
    ...
    for (unsigned i = 0; i <= lits.size(); i++) {
      if (i != 0) {
        ...
        if (lits.size() > 1 && !m_solver->solve(connected)) {
          addMonotoneClause();
          break;
        }
      }
      ...
      // TODO print the current smt connected component and cnf connected component is -v > 9
 ...
 public:
  ...
  unsigned long cachePositiveHits() const {
    return m_cache ? m_cache->getNbPositiveHit() : 0;
  }

  unsigned long cacheNegativeHits() const {
    return m_cache ? m_cache->getNbNegativeHit() : 0;
  }
};
