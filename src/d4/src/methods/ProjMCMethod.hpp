...
class ProjMCMethod : public MethodManager {
 private:
  struct CoupleNotProjClauseSelector {
    ...
    void display() {
      // m_out << selector << " : ";
      // for (const auto &l : clause) m_out << l << " ";
      // m_out << "\n";
    }
  };
 ...
 public:
  ...
  ProjMCMethod(const OptionProjMcMethod &options, ProblemManager *problem,
    ...
    m_cacheActivated = options.optionCache.isActivated;
...
*/
  inline void showInter(std::ostream &out) {
    ...
    if (m_cacheActivated) {
      out << std::setw(c_WIDTH_PRINT_COLUMN) << m_cache->getNbPositiveHit()
          << "|" << std::setw(c_WIDTH_PRINT_COLUMN)
          << m_cache->getNbNegativeHit() << "|" << std::setw(c_WIDTH_PRINT_COLUMN)
          << m_cache->usedMemory() << "|";
    } else {
      out << std::setw(c_WIDTH_PRINT_COLUMN) << 0 << "|"
          << std::setw(c_WIDTH_PRINT_COLUMN) << 0 << "|"
          << std::setw(c_WIDTH_PRINT_COLUMN) << 0 << "|";
    }
    out << std::setw(c_WIDTH_PRINT_COLUMN) << m_nbSplit << "|"
  ...
  */
  inline void printFinalStats(std::ostream &out) {
    ...
    */
    out << "c [PROJMC]\n";
    if (m_cacheActivated) m_cache->printCacheInformation(out);
 ...
 public:
  ...
  T run() {
    T res = compute(m_out);
    m_out << "c [PROJMC] Final result: " << res << "\n";
    printFinalStats(m_out);
    return res;
  }  // run

  unsigned long cachePositiveHits() const {
    auto *dpll = dynamic_cast<DpllStyleMethod<T, T> *>(m_counter);
    return dpll ? dpll->cachePositiveHits() : 0;
  }

  unsigned long cacheNegativeHits() const {
    auto *dpll = dynamic_cast<DpllStyleMethod<T, T> *>(m_counter);
    return dpll ? dpll->cacheNegativeHits() : 0;
  }
};
}  // namespace d4
