...
class CnfManager : public FormulaManager {
 protected:
  ...
  std::vector<uint8_t> m_markView;

  inline void resetUnMark() {
    for (auto &idx : m_mustUnMark) m_markView[idx] = 0;
    m_mustUnMark.resize(0);
  }  // resetUnMark

  inline Var findRoot(Var v) {
    Var root = v;
    while (root != m_infoCluster[root].parent) {
      root = m_infoCluster[root].parent;
    }
    while (v != root) {
      Var parent = m_infoCluster[v].parent;
      m_infoCluster[v].parent = root;
      v = parent;
    }
    return root;
  }
  ...
  std::vector<cvc5::Term> m_residualAssertions;
  std::vector<std::vector<std::string>> m_residualAssertionVars;
  std::unordered_set<std::string> m_residualVarNames;
  std::unordered_set<std::string> m_residualNameScratch;
 ...
 public:
  ...
  inline bool isFreeVariable(Var v) override {
