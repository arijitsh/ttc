...
void WrapperGlucose::addClause(const std::vector<Lit> &clause) {
  // Glucose also requires that clauses are added at level 0. Explicitly
  // backtrack to the root level before injecting the learned clause.
  s.cancelUntil(0);
  Glucose::vec<Glucose::Lit> lits;
  for (const auto &l : clause) lits.push(Glucose::mkLit(l.var(), l.sign()));
  s.addClause(lits);
}
