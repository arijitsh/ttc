...
Term SmtApproxMc::generate_boolean_hash()
{
  ...
  bool bool_to_bv = d_slv->getOptions().counting.forcecms;
  TermManager& tm = d_slv->getSolver()->getTermManager();
  Solver solver(tm);
  uint hash_val = Random::getRandom().pick(0, 1);
  Term xorcons, xbv, bv_var, temp_a, temp_b, temp_c, one;
  Sort bv1;
  one = tm.mkBitVector(1, 1);

  if (bool_to_bv){
    xorcons = tm.mkBitVector(1, hash_val);
    bv1 = tm.mkBitVectorSort(1);
  }
  else{
    xorcons = tm.mkBoolean(hash_val);
  }
  ...
  for (cvc5::Term x : booleans_in_projset)
  {
    ...
    if (Random::getRandom().pick(0, 1) == 1)
    {
      if (bool_to_bv)
      {
        bv_var = tm.mkConst(bv1, "ttcBool_"+x.getSymbol());
        temp_a = tm.mkTerm(cvc5::Kind::EQUAL, {bv_var, one});
        temp_b = tm.mkTerm(cvc5::Kind::EQUAL, {temp_a, x});
        Trace("smap-hash") << "Asserting " << temp_a <<"\n";

        // d_slv->getSolver()->assertFormula(temp_a);
        Trace("smap-hash") << "Asserting " << temp_b << "\n";
        d_slv->getSolver()->assertFormula(temp_b);
        Trace("smap-hash") << "Done\n";
        xorcons = tm.mkTerm(cvc5::Kind::BITVECTOR_XOR, {xorcons, bv_var});
      }
      else {
  ...
  if (bool_to_bv)
  {
    xorcons = tm.mkTerm(cvc5::Kind::EQUAL, {xorcons, one});
  }
  return xorcons;
}
  ...
  while (true)
  {
    ...
    if (numHashes > oldhashes)
    {
      ...
      for (int i = oldhashes; i < numHashes; ++i)
      {
        ...
        Trace("smap-hash") << "Asserting " << hash << "\n";
